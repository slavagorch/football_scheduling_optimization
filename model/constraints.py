...
class ConstraintsBuilder():
    ...
    # maximum one game for each team per week
    def build_max_one_game_per_week_constr(self, m):
        def _max_one_game_per_week_rule(m, team_i, weak_k):
            return sum(m.is_match_this_week_var[team_i, team_j, weak_k]
                       for team_j in m.teams_range_set) <= 1

        m.max_one_game_per_week_constr = pe.Constraint(m.teams_range_set, m.weeks_range_set,
                                                   rule=_max_one_game_per_week_rule)
        return m
